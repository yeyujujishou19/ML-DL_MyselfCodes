...
with tf.name_scope('Conv3'):
    ...
    with tf.name_scope('conv2d_3'):
        conv2d_3 = conv2d(h_pool2, W_conv3) + b_conv3  #batch_size*8*8*64

    # 加入BN层，批标准化20190707
    with tf.name_scope('BN'):
        axis = list(range(len(conv2d_3.get_shape()) - 1))
        wb_mean, wb_var = tf.nn.moments(conv2d_3, axis)

        scale = tf.Variable(tf.ones([64]))
        offset = tf.Variable(tf.zeros([64]))
        variance_epsilon = 0.001
        conv2d_3_BN = tf.nn.batch_normalization(conv2d_3, wb_mean, wb_var, offset, scale, variance_epsilon)

    with tf.name_scope('relu3'):
        # relu3 = tf.nn.relu(conv2d_3)
        relu3 = tf.nn.leaky_relu(conv2d_3_BN)
