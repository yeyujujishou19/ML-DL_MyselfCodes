...
with tf.Session() as sess:
    ...
    while step < global_steps:
        ...
        for iNo in range(inpyNUM):  # npy文件数量
            # 读取数据
            train_data_npyPath = (r"E:/sxl_Programs/Python/CNN/npy/Img4_features_train_%d.npy" % (iNo + 1))  # npy路径
            train_labels_npyPath = (r"E:/sxl_Programs/Python/CNN/npy/Img4_labels_train_%d.npy" % (iNo + 1))  # npy路径
            ...
            for flag in range(len_train_data):  # 对当前训练集逐个获取batch_size块的数据
            ...
            if iCount % 6 == 0:
                ...
                for iNo in range(inpyNUM):  # npy文件数量
                    ...
                    test_data_npyPath = (r"E:/sxl_Programs/Python/CNN/npy/Img4_features_test_%d.npy" % (iNo + 1))  # npy路径
                    test_labels_npyPath = (r"E:/sxl_Programs/Python/CNN/npy/Img4_labels_test_%d.npy" % (iNo + 1))  # npy路径
                    ...
                    sum_train_acc = 0
                    len_train_data = int(ceil(len(train_data) / batch_size))  # ceil(3.01)=4  向上取整
                    for iNo in range(len_train_data):  # 对当前训练集逐个获取batch_size块的数据
                    ...
                    sum_train_acc = sum_train_acc / len_train_data
                    all_sum_train_acc=all_sum_train_acc+sum_train_acc


                    # 测试集正确率
                    sum_test_acc = 0
                    len_test_data = int(ceil(len(test_data) / batch_size))  # ceil(3.01)=4  向上取整
                    for iNo in range(len_test_data):  # 对当前训练集逐个获取batch_size块的数据
                    ...
                    sum_test_acc = sum_test_acc / len_test_data
                    all_sum_test_acc=all_sum_test_acc+sum_test_acc

                all_sum_train_acc = all_sum_train_acc/inpyNUM
                all_sum_test_acc = all_sum_test_acc/inpyNUM
                print("Iter " + str(step) + ",Training Accuracy=" + str(all_sum_train_acc))
                print("Iter " + str(step) + ",Testing Accuracy=" + str(all_sum_test_acc))