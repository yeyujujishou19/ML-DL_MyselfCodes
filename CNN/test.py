...
with tf.Session() as sess:
    ...
    for i in range(1001):
        ...
        # 读取数据
        train_data = (r"E:/sxl_Programs/Python/CNN/npy/Img10_features_train_1.npy")  # npy路径
        train_labels = (r"E:/sxl_Programs/Python/CNN/npy/Img10_labels_train_1.npy")  # npy路径

        test_data = (r"E:/sxl_Programs/Python/CNN/npy/Img10_features_test_1.npy")  # npy路径
        test_labels = (r"E:/sxl_Programs/Python/CNN/npy/Img10_labels_test_1.npy")  # npy路径
        ...
        batch_xs, batch_ys = train_data[flag * batch_size:(flag + 1) * batch_size], \
                             train_labels[flag * batch_size:(flag + 1) * batch_size]  # 获取一块最小数据集
        ...
        batch_xs, batch_ys = test_data[flag * batch_size:(flag + 1) * batch_size], \
                             test_labels[flag * batch_size:( flag + 1) * batch_size]  # 获取一块最小数据集
        batch_ys_new = InitImagesLabels(batch_ys)  # 从数字标签转换为数组标签 [0,0,0,...1,0,0]
        summary = sess.run(merged, feed_dict={x: batch_xs, y: batch_ys_new, keep_prob: 1.0})
        test_writer.add_summary(summary, i)

        if i % 100 == 0:
            train_labels_new = InitImagesLabels(train_labels)  # 从数字标签转换为数组标签 [0,0,0,...1,0,0]
            test_labels_new = InitImagesLabels(test_labels)  # 从数字标签转换为数组标签 [0,0,0,...1,0,0]
            test_acc = sess.run(accuracy, feed_dict={x: test_data, y: test_labels_new, keep_prob: 1})
            train_acc = sess.run(accuracy, feed_dict={x: train_data, y: train_labels_new, keep_prob: 1.0})
